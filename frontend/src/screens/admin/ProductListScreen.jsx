      ...
      <Row className="align-items-center">
        ...
        {userInfo && userInfo.role === "admin" && (
          <Col className="text-end">
            <Button variant="warning" className="btn-sm m-3" onClick={createProductHandler}>
              <FaEdit /> Create Product
            </Button>
          </Col>
        )}
      </Row>
      {(isLoading || loadingCreate || loadingDelete) ? <Loader /> : error ? 
        <Message variant="danger">{error.message || (error.status === 401 && "You are not authorized to access this page.")}</Message> : (
        <>
                  ...
                  {userInfo && userInfo.role === "admin" && (
                    <td>
                      <LinkContainer to={`/admin/products/${product._id}/edit`}>
                        <Button variant="light" className="btn-sm x-2">
                          <FaEdit />
                        </Button>
                      </LinkContainer>
                      <Button variant="light" className="btn-sm px-2" style={{ "color": "black" }} onClick={() => deleteHandler(product._id)}>
                        <FaTrash />
                      </Button>
                    </td>
                  )}
